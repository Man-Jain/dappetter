...
export async function getMetamask(
  ...
  return {
    ...
    addNetwork: async ({
      ...
      const nameInput = await metamaskPage.waitForSelector(
        "input#network-name"
      );
      await nameInput.type(chainName);
      const newRPCInput = await metamaskPage.waitForSelector("input#rpc-url");
      await newRPCInput.type(rpcUrl);
      const chainIdInput = await metamaskPage.waitForSelector("input#chainId");
      await chainIdInput.type(chainId);

      const saveButton = await metamaskPage.$("#save-button");
    ...
    importPK: async (pk) => {
      ...
      const PKInput = await metamaskPage.waitForSelector(
    ...
    switchNetwork: async (network = "main") => {
      ...
      const networkIndex = await metamaskPage.evaluate(async (network) => {
        const elements = document.querySelectorAll("li.dropdown-menu-item");
    ...
    sign: async () => {
      ...
      // click on this button data-testid="home__activity-tab"

      // click on button with attribute "data-testid=rk-wallet-option-metaMask"
      await metamaskPage.waitForSelector('[data-testid="home__activity-tab"]');
      const activityTab = await metamaskPage.$(
        '[data-testid="home__activity-tab"]'
      );
      await activityTab.click();

      await metamaskPage.waitForSelector(".transaction-list-item--unconfirmed");
      const deployWalletButtonModal = await metamaskPage.$(
        ".transaction-list-item--unconfirmed"
      );
      await deployWalletButtonModal.click();

      const confirmButtonSelector = "#sign-button";
    ...
    approve: async ({ allAccounts = false }) => {
      ...
      if (allAccounts) {
        ...
        const accountListElements = await metamaskPage.$$(
          accountListElementsSelector
        );
        ...
        if (accountListElements.length > 1) {
          ...
          const allAccountsCheckbox = await metamaskPage.waitForSelector(
      ...
      const permissionApprovalButton = await metamaskPage.waitForSelector(
        permissionApprovalSelector
      );
...
async function confirmWelcomeScreen(metamaskPage: puppeteer.Page) {
  const continueButton = await metamaskPage.waitForSelector(
    ".welcome-page button"
  );
